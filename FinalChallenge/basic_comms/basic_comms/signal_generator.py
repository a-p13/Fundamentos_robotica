...
class My_Publisher(Node):
    ...
    def timer_callback(self):
        ...
        t = self.frequency_value * self.start_time
        
        if self.current_signal == 1:
            msg.data = self.amplitude_value * math.sin(2 * math.pi * self.frequency_value * t) + self.offset_value
            self.get_logger().info('Sine')
        
        elif self.current_signal == 2:
            msg.data = self.amplitude_value * sp.signal.square(2 * math.pi * self.frequency_value * t) + self.offset_value
            self.get_logger().info('Square')
        
        elif self.current_signal == 3:
            msg.data = self.amplitude_value * sp.signal.sawtooth(2 * math.pi * self.frequency_value * t) + self.offset_value
